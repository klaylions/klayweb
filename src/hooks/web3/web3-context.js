...
const DEFAULD_NETWORK = 8217;
const Web3Context = React.createContext(null);

const getMainnetURI = () => {
  return "https://kaikas.baobab.klaytn.net:8651";
...
export const Web3ContextProvider = ({ children }) => {
  ...
  const [klipRequestKey, setKlipRequsetKey] = useState("");
  const [provider, setProvider] = useState(new Caver(getMainnetURI()));
  const [providerWrapper, setProviderWrapper] = useState(
    new Caver(window?.klaytn || getMainnetURI())
  );
  // ProviderWrapper.fromWeb3Provider(new StaticJsonRpcProvider(getMainnetURI()))
  ...
  const connect = useCallback(
    async (option) => {
      ...
      const rawProvider = window.klaytn;
      const accounts = await window.klaytn.enable();
      setAddress(accounts[0]);
      ...
      if (rawProvider.isKaiKas) {
        connectedProvider = new Caver(window.klaytn);
        //   } else if (rawProvider.isKlip) {
        //     connectedProvider = ProviderWrapper.fromKlip(rawProvider);
        //   } else {
        //     connectedProvider = ProviderWrapper.fromWeb3Provider(
